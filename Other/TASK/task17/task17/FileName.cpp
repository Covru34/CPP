...
public:
	...
	void removeCar(string model)
	{
		for (auto it = cars.begin(); it != cars.end(); it++)
		{
			...
			{
				cars.erase(it); // Автоматично зсуває елемент та зменшує розмір
				cout << "Автомобіль " << model << " видаленно\n";
				return;
			}
		}
	...
	void saveToFile(string filename)
	{
		...
		outFile << cars.size(); // записуємо кількість машин першим рядком
	...
	void loadFromFile(string filename)
	{
		...
		cars.clear(); // Очищаємо вектор перед завантаженням нових даних
